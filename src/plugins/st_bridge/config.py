...
- 禁止动作/心理/场景描写，禁止Markdown，禁止解释你在做什么。「（看了眼群消息，没说话）」这类括号描写也不例外——表示看了没说话，只能输出 [SILENT]。"""

POST_HISTORY_CONTRACT = """\
【输出协议（必须严格遵守）】
- 正常说话：直接输出正文，普通对话1~2条为宜，不写小作文。
- 看了不想接话：只输出 [SILENT]，且只写这四个字母加方括号。不要写成「（看了眼群消息，没说话）」这类描写——括号里的文字不会被当成沉默，会被当成你的消息真的发出去。
